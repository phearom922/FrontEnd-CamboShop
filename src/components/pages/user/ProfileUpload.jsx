...
const ProfileUpload = ({ values, setValues, loading, setLoading }) => {
      ...
      for (let i = 0; i < files.length; i++) {
        Resize.imageFileResizer(
          ...
          (uri) => {
            axios
              .post(
                import.meta.env.REACT_APP_KEY + "/images-profile",
                {
                  image: uri,
                },
                {
  ...
  const handleRemove = (public_id) => {
    ...
    axios
      .post(
        import.meta.env.REACT_APP_KEY + "/removeimages-profile",
        { public_id },
        {
          headers: {
            Authorization: `Bearer ${user.token}`,
  ...
  return (
    <div className="flex w-2xl gap-2">
      <label
        className="relative flex h-35 w-35 cursor-pointer flex-col items-center justify-center rounded-full border border-dashed border-gray-400 bg-gray-100 hover:border-blue-800 hover:bg-gray-300"
      ...
      >
        ...
        <span className="text-sm text-gray-400 select-none">{`${loading ? "Uploading..." : "Upload Photo "}`}</span>
        <input
